...
pub async fn get_shortlink(
    ...
    // 意思是不知道泛型的函数返回值是什么类型,因为他的返回值RV没有指定值类型
...
pub async fn create_shortlink(
    ...
    match shortlink::create_shortlink(&state.pool, &req.url).await {
        Ok(r) => {
            tracing::info!("MySql query result{:?}", r);
            (StatusCode::OK, Json(shortlinkdto::CreateUserResp { ok: true }))
        },
        Err(_) => (
